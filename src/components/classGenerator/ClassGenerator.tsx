...
const ClassGenerator = () => {
    ...
    const [teacherName, setTeacherName] = useState<TeacherDataProps>({} as TeacherDataProps);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const classDataWithTeachers = resultClass.data.ClassName.map((classItem: classDataProps) => {
                    const matchedTeacher = teacherData.find(
                        (teacher) => teacher.teacher_id === classItem.id_teacher
                    );
                    return {
                        ...classItem,
    ...
    return (
        <Box sx={{ padding: 3, paddingLeft: 0, width: '80vw' }}>
            ...
            <div className='flex h-[80vh] flex-col gap-4 rounded-3xl p-5 text-[#0c427770] shadow-md'>
                ...
                <div className='flex justify-end'>
                    <Link href={'/generator/class/finalize'} className='rounded-md bg-[#31426E] p-2 px-4 text-white' >Generate Class</Link>
                </div>
            </div>
            {/* </div> */}
        </Box>
